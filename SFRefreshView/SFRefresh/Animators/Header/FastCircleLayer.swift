...
@objc open class FastCircleLayer: CALayer {
    ...
    init(frame: CGRect, color: UIColor = UIColor.coler(hex: 0xffd52b), pointColor: UIColor = UIColor.coler(hex: 0x333333), lineWidth: CGFloat = 1) {
        self.color      = color
    ...
    @objc open func endAnimation(finish: Bool) {
        if finish {
            stop = false
            rotated       = 0
            rotatedSpeed  = 0
    ...
    private func addCircleAnimation() {
        ...
        groups.duration = 2
        circle.add(groups, forKey: "Circle")
    }
    
    
    private func addCheckLayer() {
        check = FastCheckLayer(frame: CGRect(x: 0, y: 0, width: frame.size.width, height: frame.size.height), color: pointColor, lineWidth: 2.5)
        addSublayer(check!)
    }
}
